...
def collate_batch(batch):
    """
    각 배치의 샘플은 (node_features, adj_matrix, edge_index, edge_attr, target)
    형태로 구성되어 있습니다.
    """
    ...
    padded_edge_indices = []
    padded_edge_attrs = []
    # edge_indices가 2차원이 아니면 0으로 간주
    max_edges = max(ei.size(1) if ei.dim() > 1 else 0 for ei in edge_indices)
    for ei, ea in zip(edge_indices, edge_attrs):
        ...
        if pad_size > 0:
            pad_ei = torch.full((2, pad_size), -1, dtype=torch.long)  # 패딩 값은 -1
    ...
    # 텐서 스택
    node_features_tensor = torch.stack(padded_node_features)
    adj_matrices_tensor = torch.stack(padded_adj_matrices)
    edge_indices_tensor = torch.stack(padded_edge_indices)
    edge_attrs_tensor = torch.stack(padded_edge_attrs)
    node_masks_tensor = torch.stack(node_masks).bool()
    targets_tensor = torch.stack(targets)
    
    # smiles는 리스트 그대로 반환 (string list)
    return {
        "node_features": node_features_tensor,
        "adj_matrices": adj_matrices_tensor,
        "edge_indices": edge_indices_tensor,
        "edge_attrs": edge_attrs_tensor,
