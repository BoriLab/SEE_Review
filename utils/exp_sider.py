...
VERBOSE = True
MODEL_SAVE_DIR = "best_models_sider"
DEVICE = ""
...
class Aliquot:
    ...
    def _adjust_target(self, y):
        # target의 shape가 [batch, 1, num_tasks] 또는 [batch, num_tasks, 1]인 경우, 올바른 shape로 조정
        if y.dim() == 3:
            if y.size(1) == 1:
                y = y.squeeze(1)  # [batch, 1, num_tasks] -> [batch, num_tasks]
            elif y.size(-1) == 1:
                y = y.squeeze(-1)  # [batch, num_tasks, 1] -> [batch, num_tasks]
        return y

    def _loop(self, bat):
        bat = [tensor.to(self.device) for tensor in bat]
        y = self._adjust_target(bat[-1])
