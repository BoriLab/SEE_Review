...
VERBOSE = True
MODEL_SAVE_DIR = "best_models_tox21pyg"
DEVICE = ""
...
def compute_roc_auc_metrics(predictions, targets):
    ...
    (Tox21: 다중 레이블 분류의 경우 micro-average ROC-AUC)
...
class Aliquot:
    ...
    def _loop(self, bat):
        ...
        y = bat[-1]
        # Tox21의 경우 target의 shape가 [batch, 1, num_tasks]면 squeeze(1)로 [batch, num_tasks]로 맞춥니다.
        if y.dim() == 3 and y.size(1) == 1:
            y = y.squeeze(1)
        y_hat = self.model(*bat[:-1])
        # NaN 마스크 적용: NaN이 아닌 값만 사용
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for tup in self.eval_dataloader:
                ...
                # 평가 ROC-AUC 계산을 위해 valid한 예측 및 타겟만 저장
