...
VERBOSE = True
MODEL_SAVE_DIR = "best_models_clintox"
DEVICE = ""
...
def compute_roc_auc_metrics(predictions, targets):
    ...
    (Clintox용, 다중 라벨 분류: 예측값의 shape는 (batch, 2))
    ...
    Returns:
        auc (float): ROC-AUC 스코어 (micro-average)
    """
    ...
    if preds_prob.ndim == 1 or preds_prob.shape[1] == 1:
        ...
        try:
            auc = roc_auc_score(targets_np, preds_prob, average="micro")
        except Exception as e:
            auc = 0.5
    return auc
...
class Aliquot:
    ...
    def _loop(self, bat):
        ...
        if isinstance(bat, dict):
            node_features = bat["node_features"].to(self.device)
            adj_matrices = bat["adj_matrices"].to(self.device)
            edge_indices = bat["edge_indices"].to(self.device)
            edge_attrs = bat["edge_attrs"].to(self.device)
            node_masks = bat["node_masks"].to(self.device)
            targets = bat["targets"].to(self.device)
            y = targets
            # 타겟 텐서의 차원이 3이면 무조건 squeeze(1) 적용
            if y.dim() == 3:
                y = y.squeeze(1)
            y_hat = self.model(node_features, adj_matrices, edge_indices, edge_attrs, node_masks)
        else:
            new_bat = [tensor.to(self.device) for tensor in bat]
            y = new_bat[-1]
            if y.dim() == 3:
                y = y.squeeze(1)
            y_hat = self.model(*new_bat[:-1])
        ...
        with torch.no_grad():
            for tup in self.eval_dataloader:
                if isinstance(tup, dict):
                    batch = {k: (v.to(self.device) if isinstance(v, torch.Tensor) else v) for k, v in tup.items()}
                    y = batch["targets"]
                    if y.dim() == 3:
                        y = y.squeeze(1)
                    y_hat = self.model(
                    ...
                    new_tup = [tensor.to(self.device) for tensor in tup]
                    y = new_tup[-1]
                    if y.dim() == 3:
                        y = y.squeeze(1)
                    y_hat = self.model(*new_tup[:-1])
                    batch_size = new_tup[-1].size(0)
                loss = self.criterion(y_hat, y)
        ...
        for epoch in range(num_epochs):
            ...
            for step, tup in enumerate(self.train_dataloader):
                ...
                if self.scheduler is not None:
                    self.scheduler.step()
                # 배치 크기 결정: 딕셔너리이면 "targets"에서, 그렇지 않으면 첫 번째 요소에서
                if isinstance(tup, dict):
                    batch_size = tup["targets"].size(0)
                else:
