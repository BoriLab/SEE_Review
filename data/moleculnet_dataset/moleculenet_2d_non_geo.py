...
def adj_from_edge_index(edge_index: torch.Tensor, num_nodes: int):
    A = np.zeros((num_nodes, num_nodes), dtype=np.float32)
    for i, j in edge_index.t().tolist():
        A[i, j] = A[j, i] = 1.0
    return A

def spectral_edge_encoding_from_adj(A, k_eigen=5, gamma=0.5):
    """
    A: (n×n) symmetric adjacency (0/1 for bonds)
    ...
    # Compute smallest nonzero k_eigen+1 eigenpairs, skip the zero mode if desired
    # Here we take the FIRST k_eigen eigenpairs (including 0th if isolated)
    if n > k_eigen + 1:
        # faster for sparse L
        eigvals, eigvecs = eigsh(L, k=k_eigen+1, which='SM')
        # drop the first if it's numerically ≈0
        eigvals, eigvecs = eigvals[1:k_eigen+1], eigvecs[:, 1:k_eigen+1]
    else:
        eigvals, eigvecs = np.linalg.eigh(L)
        eigvals, eigvecs = eigvals[:k_eigen], eigvecs[:, :k_eigen]

    # Build edge list & attributes
    ...
    for i in range(n):
        for j in range(i+1, n):
            if A[i, j]:
                # ΔL for this edge
                delta = np.zeros((n, n))
                delta[i, i] = delta[j, j] = 1
                delta[i, j] = delta[j, i] = -1
                # sensitivity per eigenmode
                delta_lambda = eigvecs.T @ delta @ eigvecs  # shape k_eigen×k_eigen
                # sum over modes weighted by spectrum
                ESS = np.sum(np.exp(-gamma * eigvals) * np.abs(np.diag(delta_lambda)))
                edge_index += [[i, j], [j, i]]
                edge_attr  += [[ESS], [ESS]]
    ...
    for data in dataset:
        ...
        if sample["smiles"] is not None:
            ...
            if coords is not None:
                ...
                # assume `data.edge_index` and `data.num_nodes` exist
                A = adj_from_edge_index(data.edge_index, data.num_nodes)
                sample["edge_index_2d"], sample["edge_attr_2d"] = spectral_edge_encoding_from_adj(
                    A, k_eigen=5, gamma=0.5)
            else:
                sample["coords"] = None
                sample["adj"] = None
...
if __name__ == "__main__":
    ...
    for dset in datasets:
        ...
        pkl_path = f"/root/2025/sse_moleculenet/data/moleculenet_dataset/data_pkl/{dset.lower()}_data_non.pkl"   # 예: bbbp_data.pkl, tox21_data.pkl 등
...
# pkl_file = '/root/2025/sse_moleculenet/data/moleculenet_dataset/data_pkl/bbbp_data.pkl'
